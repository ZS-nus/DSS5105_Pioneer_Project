...
export default function GBarChart(props) {
  ...
  const { chartData, labels, latestYear } = useMemo(() => {
    ...
    const { labels, data: chartData } = g_barChartData(data);
  ...
  const chartOptions = {
    ...g_barChartOptionsDailyTraffic,
    xaxis: {
      ...g_barChartOptionsDailyTraffic.xaxis,
      categories: labels,
    },
  };
        ...
        <Flex flexDirection='column' align='start' me='20px'>
          <Flex w='100%'>
            <Text
              ...
              fontWeight='700'
              lineHeight='100%'>
              Governance Rating {latestYear ? `(${latestYear})` : ''}
            </Text>
          </Flex>
        </Flex>
      </Flex>

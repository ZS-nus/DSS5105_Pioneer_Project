...
def analyze_trend_with_template(esg_scores_df, company_id, company_info_df):
    try:
        ...
        print("Available columns:", esg_scores_df.columns.tolist())
        ...
        pred_env = predicted_group['Environmental_Score'].values if not predicted_group.empty else []
        pred_soc = predicted_group['Social_Score'].values if not predicted_group.empty else []
        pred_gov = predicted_group['Governance_Score'].values if not predicted_group.empty else []
        ...
        return "An error occurred while analyzing the trend."
